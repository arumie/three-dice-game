 ...
 */
export function calculatePenaltyFromTurns(turns: PlayerTurnModel[]): number {
 ...
 */
export function findLosersFromTurns(turns: PlayerTurnModel[]): number[] {
...
export function computeScoreToBeat(
  ...
  const candidates = turns.filter(
    (t) =>
      t.participantId !== excludeParticipantId &&
      !t.isSafe &&
      t.finalScore !== null,
  );
  ...
  return {
    score: lowest.finalScore as number,
    participantId: lowest.participantId,
  };
}
