...
function SipsAtStake({ sips }: { sips: number }) {
	...
	return (
		<div className={`mx-4 my-3 flex items-center gap-3 rounded-lg border px-3 py-2.5 sm:mx-5 ${config.bg} ${config.border}`}>
			<Icon className={`size-5 shrink-0 ${config.text} ${severity === "extreme" ? "animate-pulse" : ""}`} />
			<div className="flex items-baseline gap-1.5">
				<span className={`text-2xl font-bold tabular-nums leading-none ${config.numColor}`}>
					{sips}
				</span>
...
export function RoundInfoCard({
	...
	const completedScores = round.turns
		...
		.map((t) => ({ participantId: t.participantId, score: t.finalScore as number }));
	const lowestScore = completedScores.length > 0
		? Math.min(...completedScores.map((s) => s.score))
		: null;
	const losingParticipantIds = new Set(
		completedScores.filter((s) => s.score === lowestScore).map((s) => s.participantId),
	);
